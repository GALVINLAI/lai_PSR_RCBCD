...
def gradient_descent(f, initial_point, learning_rate, num_iterations, sigma):
    ...
    return function_values, x, function_value
...
def main(p, sigma, repeat):
    # try the gradient descent algorithm
    learning_rate = 0.01


    make_dir('exp_noise')
    make_dir(f'exp_noise/p_{p}')
    for exp_i in range(repeat):
        ...
        function_values1, x1, f_x1 = gradient_descent(get_reward, x, 0.001, num_iterations, sigma)

        # Run random coordinate descent
        x2, f_x2, function_values2 = random_coordinate_descent(
            get_reward, x, 0.01, num_iterations, sigma
        )
        
        make_dir(f'exp_noise/p_{p}/sigma_{sigma}/exp_{exp_i}')
        data_dict = {
        ...
        dump(data_dict, f'exp_noise/p_{p}/sigma_{sigma}/exp_{exp_i}/data_dict.pkl')
