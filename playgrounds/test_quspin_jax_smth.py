...
@click.command()
@click.option("--p", default=15, help="The dimension of the problem")
def main(p):
    ...
    # key = jrd.PRNGKey(42)
    # x = jrd.uniform(key, shape=(p,), minval=0.0, maxval=1.0)
    
    # ic(get_reward(*x))
    
    # for i in range(p): 
    #     ic(second_derivative(get_reward, i)(*x))

    # # Set the number of iterations for gradient descent
    num_iterations = 1000

    # Run gradient descent
    # function_values1, x = gradient_descent(get_reward, x, 0.001, num_iterations)
    x = load('x_value.pkl')
    ...
    for i in range(p): 
        print(i, min(lip_lip_list[i]), max(lip_lip_list[i]))

    # for i in range(p):
    #     ic(i, first_derivative(get_reward2, i)(*x))

    # Run random coordinate descent
    # _, _, function_values2 = random_coordinate_descent(
    #     get_reward, x, 0.01, num_iterations
    # )
