...
def main():
    make_dir('exp/heisenberg')
    make_dir(f'exp/heisenberg/dim_{dim}')
    ...
    for exp_i in range(repeat):
        ...
        if os.path.exists(f'exp/heisenberg/lr_{lr_gd}/dim_{dim}/sigma_{sigma}/exp_{exp_i}/data_dict.pkl'):
            continue
        ...
        dump(data_dict, f'exp/heisenberg/lr_{lr_gd}/dim_{dim}/sigma_{sigma}/exp_{exp_i}/data_dict.pkl')
