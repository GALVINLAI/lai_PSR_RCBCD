...
sigma = 0.1 # the noise for the gd and rcd algorithms
...
class GradientDescent_noise(GradientDescent):
    def tell(self, ask_data: AskData, tell_data: TellData) -> None:
        ...
        noise = np.random.normal(0, 1, self.state.x.shape) * sigma 
        self.state.x = self.state.x - next(self.state.learning_rate) * (tell_data.eval_jac + noise)
...
class RandomCoordinateDescent(GradientDescent):
    ...
    def tell(self, ask_data: AskData, tell_data: TellData) -> None:
        ...
        noise = np.random.normal(0, 1, 1) * sigma 
        self.state.x[self._selected_coordinate] -= next(self.state.learning_rate) * (tell_data.eval_jac[self._selected_coordinate] + noise) 
...
if optimizer_type == 'gd':
    optimizer = GradientDescent_noise(maxiter=2000, learning_rate=learning_rate, callback=log.update)
else: 
